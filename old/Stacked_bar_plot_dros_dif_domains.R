...
ggplot(df_dros_domains, aes(x=species, y = values/100, fill = legend_val))+
  ...
  theme(plot.margin = margin(0.4, 0.2, -0.15, 0.5, "cm"))+
  theme (legend.position = c(.5,0.92), legend.direction = "horizontal",
         legend.title = element_text(size = 7.0), 
         legend.text = element_text(size = 7.0))+
  guides(shape = guide_legend(override.aes = list(size = 1.0)))+
  guides(color = guide_legend(override.aes = list(size = 1.0)))+
  scale_x_discrete(name="")+
  scale_y_continuous(label = percent, name="Classified reads (%)", limits = c(0/100, 18/100))+
  scale_fill_brewer(name = "Classification group",labels = c("archaea", "bacteria", "human", "other", "virus"), palette = "Set2")
ggsave("Classified_at_D.pdf",width=5,height = 4)  
