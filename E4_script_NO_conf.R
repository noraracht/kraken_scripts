...
# Code below should be used on fomatted input
...
qplot(cl/100,sign(error)*sqrt(abs(error)),data=dm[!dm$k %in% c(28,29,32),],linetype=Kraken,color=k,shape=k)+geom_line()+
  facet_grid(percent(round(D,3))~bin,scales="free",space = "free_x")+
  ...
  scale_x_continuous(labels=scales::percent_format(accuracy = 1),name=expression("Contamination level"~c[l]),breaks=c(0,.1,0.2,0.4,0.6))+
  scale_color_manual(values=c("#e31a1c",#"#fd8d3c",
                              "black"))+  
  scale_linetype_manual(name="",values=c(2,1),labels=c("Before Filtering","After Filtering"))+scale_shape_manual(values=c(0,2,6,19))+
