...
#ds = (read.csv("drosophilaskims_conf0.05.csv"))
names(ds)
...
#kf= (read.csv('/Users/admin/Documents/Skimming/dros_real_data/14_dros_genomes/report_dros_skmer_paper_real_kraken_std_unmasked_0.05_k35/sum_report_dros_skmer_paper_real_kraken_std_unmasked_0.05_k35.csv'))
...
#ggsave("Drosophila-errreduction_0.05.pdf",width=5,height = 4.5)
...
#ggsave("Drosophila-errreduction-all_conf0.05.pdf",width=9,height = 8)
...
#ggsave("Drosophila-filtered--error_before-red_in_error_conf0.05.pdf",width=9,height = 8)
...
ggplot(aes(fill=(d2-fna_dist)/fna_dist, 
           x=paste(s1,#percent(C_sequences_prct.x/100),
  ...
  #ggsave("Drosophila_tile_plot_conf0.05.pdf",width=9,height = 8)
...
#ggsave("Drosophila--proportion_filtered-error_before_Kraken_conf0.05.pdf",width=9,height = 8)

qplot(abs(bk_no_clean_up-fna_dist)/fna_dist-abs(bk_cleaned-fna_dist)/fna_dist,data=ds)

correctedj = function(x) {jj=x; rr=(1-jj+a/(1+h)*(h-jj))/(1+jj); (1-rr)/(1+rr)}
