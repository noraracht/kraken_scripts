...
#r=read.csv("export_df_all_data_extended_git.csv")
r=read.csv("export_df_all_data_extended_v2.csv")
names(r)
options(digits=2)
...
ggplot(aes(x=FPR,y=recall,size=confidence_level,color=k,group=k),
  ...
  scale_color_gradient(guide="legend",high="#01237c",low = "#a6cbdf", breaks=c(23,26,28,32,35))+
  scale_size_continuous(name=expression(alpha),range = c(1.8,0.4), breaks=c(0,0.05,0.1,0.2,0.3))+
  theme_classic() +
